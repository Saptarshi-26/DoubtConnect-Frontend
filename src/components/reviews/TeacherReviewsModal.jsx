...
/**
 * Modal that fetches and displays all reviews left for a given teacher.
 *
 * Props:
 * - teacherId: number
 * - teacherName: string
 * - onClose: () => void
 */
function TeacherReviewsModal({ teacherId, teacherName, onClose }) {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-lg font-bold text-slate-800">Reviews</p>
            {teacherName && (
              <p className="text-sm text-slate-400">for {teacherName}</p>
            )}
          </div>
          ...
          {!loading && !loadError && reviews.length === 0 && (
            <div className="flex flex-col items-center rounded-2xl bg-slate-50 p-10 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Reviews from students will show up here.
              </p>
            </div>
          )}
