...
function DeleteStudentPage() {
  ...
  const handleDelete = async () => {
    ...
    setError("");
    try {
      await api.delete(`/student/${studentProfileId}`);
      localStorage.clear();
      ...
      <div className="mx-auto max-w-2xl px-6 py-10 sm:px-8 sm:py-14">
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-8 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_16px_32px_-12px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none sm:p-10">
          ...
          <p className="mt-3 leading-7 text-slate-600 dark:text-slate-400">
            This will permanently delete your DoubtConnect account. Any
            pending session requests will be cancelled, and any upcoming
            sessions will be cancelled and released back to the educator's
            availability. This action cannot be undone.
          </p>

          <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-white/[0.03]">
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Consider exporting any session notes or history first — once
